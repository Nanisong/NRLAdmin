    ...
    public class Ladder
    {
       ...
       //             cmd.CommandText = @"SELECT 
				...
				cmd.CommandText = @"SELECT 
							...
							SUM(CASE WHEN (G.HomeTeamID = T.TeamID AND G.HomeTeamScore > G.AwayTeamScore) OR 
										  (G.AwayTeamID = T.TeamID AND G.AwayTeamScore > G.HomeTeamScore) THEN 3 
									 WHEN G.HomeTeamScore = G.AwayTeamScore THEN 1 
									 ELSE 0 
								END) AS Points,
							SUM(CASE WHEN (G.HomeTeamID = T.TeamID AND G.HomeTeamScore > G.AwayTeamScore) OR 
										  (G.AwayTeamID = T.TeamID AND G.AwayTeamScore > G.HomeTeamScore) THEN 1 
									 ELSE 0 
								END) AS Wins,
							SUM(CASE WHEN G.HomeTeamScore = G.AwayTeamScore THEN 1 ELSE 0 END) AS Draws,
							SUM(CASE WHEN (G.HomeTeamID = T.TeamID AND G.HomeTeamScore < G.AwayTeamScore) OR 
										  (G.AwayTeamID = T.TeamID AND G.AwayTeamScore < G.HomeTeamScore) THEN 1 
									 ELSE 0 
								END) AS Losts
